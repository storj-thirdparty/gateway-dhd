...
func trimStartSlash(path string) string {
	newPath := path
...
// @Param yourMetaData formData string false "Your custommetadata" default()
// @Header 200 {string} Token "qwerty"
...
func UploadObject(c *gin.Context) {
	...
	objectPathPrefix := trimStartSlash(objectPathPrefixWithSlash)
	...
	objectPath := objectPathPrefix + separator + objectName

	// Get object custom metadata to be set from the POST request
	yourMetaData := c.PostForm("yourMetaData")
	
	// Create an empty uplink CustomMetaData object.
	// This would be used to set the custom metadata for the uploaded object
	metaData := uplink.CustomMetadata{}

	// Map data structure to parse the custom metadata input
	var parsed map[string]string

	// Obtain bytes from the entered custom metadata input
	f := []byte(yourMetaData)

	// Parse the custom metadata input as a map
	...
	// Append each key-value pair parsed from the entered metaData in the uplink CustomMetaData object
